...
LTCDecoder* ltc_decoder_create(int apv, int queue_len) {
	...
	d->snd_to_biphase_lmt = (int) (d->snd_to_biphase_period * 3) / 4;
...
LTCEncoder* ltc_encoder_create(double sample_rate, double fps, enum LTC_TV_STANDARD standard, int flags) {
	...
	e = (LTCEncoder*) calloc(1, sizeof(LTCEncoder));
	if (!e)
		return NULL;
	...
	e->bufsize = (size_t) (1 + ceil(sample_rate / fps));
...
int ltc_encoder_reinit(LTCEncoder *e, double sample_rate, double fps, enum LTC_TV_STANDARD standard, int flags) {
	...
	bufsize = (size_t) (1 + ceil(sample_rate / fps));
...
int ltc_encoder_set_volume(LTCEncoder *e, double dBFS) {
	...
	double pp;
	ltcsnd_sample_t diff;

	if (dBFS > 0)
		return -1;
	pp = rint(127.0 * pow(10, dBFS/20.0));
	if (pp < 1 || pp > 127)
		return -1;
	diff = ((ltcsnd_sample_t) pp)&0x7f;
...
int ltc_encoder_inc_timecode(LTCEncoder *e) {
	return ltc_frame_increment (&e->f, (int) rint(e->fps), e->standard, e->flags); 
}

int ltc_encoder_dec_timecode(LTCEncoder *e) {
	return ltc_frame_decrement (&e->f, (int) rint(e->fps), e->standard, e->flags);
}
...
ltc_off_t ltc_frame_alignment(double samples_per_frame, enum LTC_TV_STANDARD standard) {
	switch (standard) {
		case LTC_TV_525_60:
			return (ltc_off_t) rint(samples_per_frame * 4.0 / 525.0);
		case LTC_TV_625_50:
			return (ltc_off_t) rint(samples_per_frame * 1.0 / 625.0);
		default:
			return 0;
	}
}
